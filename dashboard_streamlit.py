...
.ka-login-hero{
background:linear-gradient(180deg,rgba(9,17,14,.96),rgba(5,9,7,.96));
border:1px solid rgba(85,255,85,.22);
border-radius:28px;
padding:34px 32px 28px;
text-align:center;
box-shadow:0 0 50px rgba(81,255,81,.08), inset 0 0 0 1px rgba(255,255,255,.03);
margin-bottom:18px;
}
.ka-login-brand{
font-size:48px;
...
letter-spacing:.05em;
margin-bottom:14px;
background:linear-gradient(90deg,#fff,#baffc5);
...
.ka-login-panel{
background:linear-gradient(180deg,rgba(6,10,11,.98),rgba(2,4,4,.98));
border-radius:22px;
...
border-radius:16px!important;
background:linear-gradient(90deg,#59ff37,#97ff3f)!important;
...
""", unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class='ka-login-wrap'>
        <div class='ka-login-hero'>
            <div class='ka-login-brand'>KANIBAL ANALYTICS</div>
            <div class='ka-login-sub'>ZAMKNIĘTY PANEL DOSTĘPU</div>
        </div>
        <div class='ka-login-panel'>
    """, unsafe_allow_html=True)

    username = st.text_input("Nazwa użytkownika", key="ka_login_user")
    password = st.text_input("Hasło", type="password", key="ka_login_pass")
