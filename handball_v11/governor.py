...
LIVE_REPORT_SCHEMA_VERSION = "handball.live_shadow_report.v1"
...
def _training_probabilities(candidate: dict) -> list[float]:
    ...
    model = HandballEloModel(**dict(artifact["hyperparameters"]))
    probabilities: list[float] = []
    rows = sorted(
...
def create_live_shadow_predictions(
    ...
    for game in games:
        ...
        champion = HandballEloModel(**champion_parameters)
