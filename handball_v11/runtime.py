...
from .storage import HandballStorage
...
def run_cycle(storage: HandballStorage, client: ApiSportsHandballClient, settings) -> dict:
    quality_policy = policy_for("handball")
    ...
    return {
        ...
        "active_model_scope": "handball_shadow_only",
        "automatic_model_promotion_allowed": integrity_training_ready,
        "automatic_model_promotion_scope": "handball_shadow_only",
...
def main() -> int:
    settings = load_handball_settings()
    if not settings.enabled:
        print("HANDBALL v11 DISABLED", flush=True)
        return 0
    storage = HandballStorage()
    storage.initialize()
    client = ApiSportsHandballClient(settings, observer=storage.record_provider_call)
    print(
        f"HANDBALL v{RUNTIME_VERSION} SHADOW START poll={settings.poll_minutes}m "
    ...
    while True:
        try:
            ...
            print(json.dumps({"event": "HANDBALL_SHADOW_CYCLE", "runtime_version": RUNTIME_VERSION, **health}), flush=True)
            ...
            print(json.dumps({"event": "HANDBALL_SHADOW_FAILED", **failure}), flush=True)
