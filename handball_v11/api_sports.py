...
from .config import HandballSettings
from .domain import OddsQuote, HandballGame, utc_now


class HandballProviderError(RuntimeError):
    pass
...
class ApiSportsHandballClient:
        ...
        for attempt in range(1, self.settings.retry_attempts + 1):
            ...
            try:
                ...
                if status_code == 429 or status_code >= 500:
                    raise HandballProviderError(f"retryable HTTP {status_code}")
                response.raise_for_status()
                payload = response.json()
                if not isinstance(payload, dict):
                    raise HandballProviderError(
                        "API-Sports returned a non-object response"
                    )
                errors = payload.get("errors")
                if errors:
                    raise HandballProviderError(f"API-Sports errors: {errors}")
                ...
                break
        raise HandballProviderError(str(last_error or "provider request failed"))
    ...
    def games_for_date(self, day: date) -> list[HandballGame]:
        rows = self._get(
            "games",
            {"date": day.isoformat(), "timezone": self.settings.timezone},
        )
        games = [HandballGame.from_api(row) for row in rows]
    ...
    def odds_for_game(self, game_id: str) -> list[OddsQuote]:
        """Return only verified two-way winner quotes.

        Regular-time 1X2 offers are rejected because silently removing their
        draw leg would create false value and poison both picks and learning.
        """
            ...
            for bookmaker in bookmakers:
                ...
                for bet in bets:
                    ...
                    if any(
                        token in normalized
                        for token in (
                            "handicap", "spread", "total", "over/under",
                            "firsthalf", "secondhalf", "teamtotal",
                        )
                    ):
                        continue
                    if not any(
                        token in normalized
                        for token in (
                            "home/away", "home-away", "winner", "moneyline",
                            "towin",
                        )
                    ):
                        continue
                    ...
                    if not isinstance(values, list):
                        continue
                    labels = {
                        str(value.get("value") or "").strip().lower()
                        for value in values
                        if isinstance(value, dict)
                    }
                    if labels & {"draw", "x", "tie", "remis"}:
                        continue
                    parsed: dict[str, OddsQuote] = {}
                    for value in values:
                    ...
                    if set(parsed) == {"HOME", "AWAY"}:
                        quotes.extend((parsed["HOME"], parsed["AWAY"]))
        return quotes
