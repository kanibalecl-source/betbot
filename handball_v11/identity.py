...
def stable_key(kind: str, source_id: str, display_name: str) -> str:
    ...
    material = f"handball|{PROVIDER}|{kind}|{identity}"
    return hashlib.sha256(material.encode("utf-8")).hexdigest()


def game_fingerprint(game: HandballGame) -> str:
    material = "|".join(
        [
...
def validate_game(game: HandballGame) -> tuple[str, ...]:
    reasons: list[str] = []
    required = {
    ...
    for label, value in (("home_goals", game.home_goals), ("away_goals", game.away_goals)):
        if value is not None and (value < 0 or value > 100):
            reasons.append(f"invalid_{label}")
    return tuple(sorted(set(reasons)))


def identity_for(game: HandballGame) -> IdentityRecord:
