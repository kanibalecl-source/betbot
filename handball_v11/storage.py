...
from .domain import OddsQuote, HandballGame, utc_now
from .features import parse_utc
from .identity import PROVIDER, identity_for, normalize_name, validate_game


def handball_data_dir(root: str | Path | None = None) -> Path:
    return Path(root) if root is not None else DATA_DIR / "handball"


class HandballStorage:
    def __init__(self, root: str | Path | None = None):
        self.root = handball_data_dir(root)
        self.db_path = self.root / "handball_shadow.sqlite3"
        ...
        with self.connect() as connection:
            connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS games (
                    ...
                    sport TEXT NOT NULL DEFAULT 'handball' CHECK(sport='handball'),
                ...
                BEGIN SELECT RAISE(ABORT, 'handball identity quarantine is append-only'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball settlement audit is append-only'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball feature snapshot is append-only'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball feature quarantine is append-only'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball pick feature link is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball market consensus is append-only'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball closing market is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball pick CLV is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball pick market link is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball training dataset is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball candidate is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball validation is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball live prediction is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball live settlement is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball live report is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball lifecycle event is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'closed handball pick is immutable'); END;
                ...
                BEGIN SELECT RAISE(ABORT, 'handball evidence is append-only'); END;
    ...
    def upsert_games(self, games: Iterable[HandballGame]) -> int:
        count = 0
        with self.connect() as connection:
            for game in games:
                ...
                connection.execute(
                    ...
                    INSERT INTO games (
                        ...
                        home_goals, away_goals, raw_json, updated_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    @staticmethod
    def _quarantine(
        connection: sqlite3.Connection, game: HandballGame, reason: str
    ) -> None:
    ...
    def load_games(self, *, finished_only: bool = False) -> list[HandballGame]:
        ...
        for row in rows:
            game = HandballGame(
                ...
                away_team=row["away_team"], home_goals=row["home_goals"],
                away_goals=row["away_goals"], raw=json.loads(row["raw_json"]),
            )
    ...
    def coverage_summary(self) -> dict:
        with self.connect() as connection:
            ...
            finished = connection.execute(
                "SELECT COUNT(*) FROM games WHERE home_goals IS NOT NULL AND away_goals IS NOT NULL"
            ).fetchone()[0]
    ...
    def register_model_candidate(self, payload: dict) -> tuple[bool, str]:
        ...
        candidate_id = f"handball_candidate_{artifact_sha[:24]}"
    ...
    def register_model_validation(self, payload: dict) -> tuple[bool, str]:
        ...
        expected_id = f"handball_validation_{expected_sha[:24]}"
        allowed_statuses = {
            "NO_ENOUGH_DATA",
            "REJECTED_OR_REVIEW",
            "POSITIVE_VALIDATION_MANUAL_APPROVAL",
    ...
    def record_live_prediction(
        ...
        *,
        ...
        prediction_key = f"handball_live_prediction_{payload_sha[:24]}"
        with self.connect() as connection:
            candidate = connection.execute(
    ...
    def settle_live_predictions(
        ...
        with self.connect() as connection:
            ...
            for row in rows:
                ...
                if game is None:
                    continue
                target = 1 if int(game.home_goals) > int(game.away_goals) else 0
                probability = min(
                    1.0 - 1e-12,
    ...
    def register_live_report(self, payload: dict) -> tuple[bool, str]:
        ...
        report_id = f"handball_live_report_{report_sha[:24]}"
        if (
    ...
    def record_lifecycle_event(
        ...
        *,
        ...
        payload_json = canonical_json(payload)
        payload_sha = sha256_text(payload_json)
        event_id = f"handball_lifecycle_{payload_sha[:24]}"
        with self.connect() as connection:
            cursor = connection.execute(
                """
    ...
    def point_in_time_training_set(
        self, target: HandballGame, observed_at: str
    ) -> tuple[list[HandballGame], dict]:
    ...
    def capture_closing_market(
        self, game: HandballGame, market: str = "MATCH_WINNER_NO_DRAW"
    ) -> sqlite3.Row | None:
        with self.connect() as connection:
    ...
    def close_pick(
        self, pick_key: str, result: str, profit: float, game: HandballGame
    ) -> bool:
        payload = json.dumps(game.raw, ensure_ascii=False, sort_keys=True)
        ...
        with self.connect() as connection:
            ...
            connection.execute(
                ...
                INSERT OR IGNORE INTO settlement_evidence (
                    evidence_key, pick_key, game_id, result, home_goals, away_goals,
                    provider, payload_sha256, payload_json, settled_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    evidence_key, pick_key, game.game_id, result, game.home_goals,
                    game.away_goals, "api-sports-handball", payload_hash, payload,
                    settled_at,
                ),
            )
        return True
    ...
    def record_settlement_audit(
        self, pick: sqlite3.Row, game: HandballGame, recalculated_result: str
    ) -> tuple[bool, str]:
        ...
        with self.connect() as connection:
            cursor = connection.execute(
                ...
                (
                    ...
                    recalculated_result, audit_status, "api-sports-handball",
