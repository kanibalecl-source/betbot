...
VALIDATION_SCHEMA_VERSION = "handball.walk_forward_validation.v2"
...
def _ordered_games(dataset_document: dict) -> list[HandballGame]:
...
def _boundaries(games: list[HandballGame]) -> list[int]:
    result = [0]
...
def build_walk_forward_folds(
...
) -> list[tuple[list[HandballGame], list[HandballGame]]]:
    ...
    folds: list[tuple[list[HandballGame], list[HandballGame]]] = []
...
def _score_fold(
    ...
    testing: list[HandballGame],
    *,
    challenger_parameters: dict,
) -> dict:
    champion = HandballEloModel(**DEFAULT_HYPERPARAMETERS)
    challenger = HandballEloModel(**challenger_parameters)
    ...
    *,
    ...
    return {
        **report,
        ...
        "validation_id": f"handball_validation_{report_sha256[:24]}",
        "validation_created": False,
    }
